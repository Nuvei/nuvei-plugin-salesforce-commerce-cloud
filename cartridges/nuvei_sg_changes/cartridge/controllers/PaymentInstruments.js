...
/* globals dw, response, session, customer */
/**
...
function list() {
    ...
    var paymentInstruments = wallet.getPaymentInstruments(require('*/cartridge/scripts/util/nuveiHelper').PAYMENT_METHOD_ID);
    var pageMeta = require('~/cartridge/scripts/meta');
    var paymentForm = app.getForm('paymentinstruments');
    var message = session.privacy.message;

    if (session.privacy.message) {
        delete session.privacy.message;
    }
...
function Delete() {
    ...
    paymentForm.handleAction({
        remove: function (formGroup, action) {
            require('*/cartridge/scripts/removeSavedCard').removeSavedCard({
                CurrentCustomer: customer,
                PaymentToDelete: action.object
            });
            ...
            session.privacy.message = 'msg.deleted.success';
        },
        error: function () {
    ...
    ResponseUtil.renderJSON({
        redirectURL: URLUtils.https('PaymentInstruments-List')
    });
}
