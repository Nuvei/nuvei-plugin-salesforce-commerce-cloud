...
var nuveiServices = require('*/cartridge/scripts/nuveiServices');
...
function start(cart, params) {
    var nuveiHelper = require('*/cartridge/scripts/util/nuveiHelper');
    ...
    params.nuveiSessionToken = nuveiServices.getSessionToken();

    var errorMessage = nuveiHelper.getErrorMessage();
...
function resetPaymentForms() {
    ...
    var status = Transaction.wrap(function () {
        if (app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value.equals('PayPal')) {
            ...
            cart.removePaymentInstruments(cart.getPaymentInstruments('NUVEI'));
            ...
            cart.removePaymentInstruments(cart.getPaymentInstruments('PayPal'));
            cart.removePaymentInstruments(cart.getPaymentInstruments('NUVEI'));
        } else if (app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value.equals('NUVEI')) {
            app.getForm('billing').object.paymentMethods.creditCard.clearFormElement();
            app.getForm('billing').object.paymentMethods.bml.clearFormElement();
        ...
        save: function () {
            Transaction.wrap(function () {
                ...
                handlePaymentSelection(cart).error) {// Performs payment method specific checks, such as credit card verification.
                    ...
                    var NuveiHelperHosted = require('*/cartridge/scripts/util/nuveiHelperHosted').getRedirectSettings(true);
                    if (NuveiHelperHosted.enabled && NuveiHelperHosted.redirect) {
                        response.redirect(NuveiHelperHosted.url);
                    } else {
