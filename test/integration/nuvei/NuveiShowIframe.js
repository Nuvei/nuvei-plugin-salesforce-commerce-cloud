...
describe('Nuvei-ShowIframe', function () {
    describe('When integration gets iframe settings', function () {
        var myRequest = {
            url: '',
            method: 'POST',
        ...
        it('should successfully get ShowIframe data', function () {
            // Generate CSRF
            myRequest.url = baseUrl + '/Nuvei-ShowIframe';
            myRequest.method = 'GET';
            return request(myRequest)
                .then(function (response) {
                    var bodyAsJson = JSON.parse(response.body);
                    assert.equal(response.statusCode, 200, 'Expected CheckoutServices-SubmitPayment statusCode to be 200.');
                    assert.isTrue(bodyAsJson.redirectSettings.enabled);
                    if (bodyAsJson.redirectSettings.mode === 'Hosted Page') {
                        assert.isNotNull(bodyAsJson.redirectSettings.url);
                    }
                });
        });
    });
